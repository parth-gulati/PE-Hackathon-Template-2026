...
def _handle_response(response):
    """Mark expected non-2xx responses as success."""
    if response.status_code in (301, 410, 429):
        response.success()
...
class URLShortenerUser(HttpUser):
    ...
    def on_start(self):
        ...
        self.short_codes = []
        with self.client.get("/urls", name="/urls", catch_response=True) as response:
    ...
    def get_url_detail(self):
        ...
        with self.client.get(f"/urls/{url_id}", name="/urls/<id>", catch_response=True) as response:
            _handle_response(response)
